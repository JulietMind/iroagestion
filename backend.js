const app = {
  ...
  renderBlog: function() {
    ...
    <div class="metric"><span class="metric-label">Rentabilidad</span><span class="metric-value gold">${(item.profit || '0%').split('/')[0]}</span><span>%</span></div>

    <!-- Añadimos '0 meses' para evitar el error -->
    <div class="metric"><span class="metric-label">Duración</span><span class="metric-value">${(item.duration || '0 meses').split(' ')[0]}</span><span>meses</span></div>

    <div class="metric"><span class="metric-label">Min. Inversión</span><span class="metric-value">${item.min || '-'}</span><span>€</span></div>
    </div>

    <!-- Enlace a articulo.php -->
    <a href="ficha-curso.php?id=${item.id}" class="card-btn">
    Ver Detalles
...
//    const tbody = document.querySelector('#admin-posts-table tbody');
 ...
 // },
  ...
  createCardsHTML: function(items) {
    ...
    <div class="metric"><span class="metric-label">Duración</span><span class="metric-value">${item.duration.split(' ')[0]}</span><span> meses</span></div>
